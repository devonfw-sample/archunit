 ...
 */
@AnalyzeClasses(packages = "com.devonfw.sample.archunit", importOptions = ImportOption.DoNotIncludeTests.class) //adjust packages?
public class ComponentRuleC7LayerBatch2Logic4ComponentTest {
    ...
    static ArchCondition<JavaClass> haveNonCompliantComponentDependencies = new ArchCondition<JavaClass> ("have dependencies, towards another components logic layer (Rule-C7).") {
        @Override
        public void check(JavaClass sourceClass, ConditionEvents events) {
            ...
            if(sourceClassLayer.equals("batch") && !sourceClassComponent.equals("")  && !sourceClassComponent.equals(DEFAULT_COMPONENT_SIMPLE_NAME)){
    ...
    /**
     * Dependency of a components batch layer towards the same components logic or common layer is allowed.
     */
    private static boolean isAllowedDependency(JavaClass sourceClass, JavaClass targetClass) {
        boolean isAllowed = false;
        String targetClassName = targetClass.getFullName();

        // Components batch layer can depend on their own lower layers: logic and common.
        if(classesAreInSameComponent(sourceClass, targetClass) && (targetClassName.contains("logic") || targetClassName.contains("common"))) {
            isAllowed = true;
        }
        // Components may always depend on the default business component.
        if(targetClassName.startsWith(DEFAULT_COMPONENT_FULL_NAME) && (targetClassName.contains("logic") || targetClassName.contains("common"))) {
            isAllowed = true;
        }
        return isAllowed;
    }
