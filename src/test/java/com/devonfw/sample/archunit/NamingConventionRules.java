...
public class NamingConventionRules {
  ...
  /**
   * DevonNamingConventionCheck N1 verifying that classes extending AbstractCto
   * are following the naming convention by
   * ending with 'Cto'.
   */
  static ArchRule namingConventionCtoCheck() {
    return classes().that().areAssignableTo(AbstractCto.class)
  ...
  /**
   * DevonNamingConventionCheck N3 verifying that classes extending
   * ApplicationPersistenceEntity are following the
   * naming convention by ending with 'Entity' and have to be in layer dataaccess.
   */
  static ArchRule namingConventionEntityCheck() {
    return classes().that()
  ...
  /**
   * DevonNamingConventionCheck N4 verifying that classes extending AbstractEto
   * are following the naming convention by
   * ending with 'Eto'n reside in package "common" and implement an interface with
   * the same simple name.
   */
  static ArchRule namingConventionEtoCheck() {
    return classes().that().areAssignableTo(AbstractEto.class)
  ...
  /**
   * DevonAbstractUcCheck verifying that classes extending AbstractUc have to have
   * be in layer logic
   */
  static ArchRule abstractUcCheck() {
    return classes().that().areAssignableTo(AbstractUc.class).should()
        .resideInAnyPackage("..logic..").because("Classes extending AbstractUc must be located in the package logic");
  }

  /**
   * DevonMapperCheck verifying that classes extending Mapper have to have be in
   * layer logic and end with the suffix
   * Mapper
   */
  static ArchRule mapperCheck() {
    return classes().that().areAnnotatedWith(Mapper.class).should()
  ...
  /**
   * DevonPathCheck verifying that classes extending Path have to have be in layer
   * service and end with the suffix
   * Service
   */
  static ArchRule pathCheck() {
    return classes().that().areAnnotatedWith(Path.class).should()
  ...
  /**
   * DevonJpaRepositoryCheck verifying that classes implementing JpaRepository
   * have to have be in layer dataaccess and
   * have to be named «EntityName»Repository where «EntityName» is the name of the
   * entity filled in the generic argument
   * of JpaRepository excluding the Entity suffix. Further they should be in the
   * same package as the entity.
   */
  static ArchRule jpaRepositoryCheck() {
    return classes().that().areAssignableTo(JpaRepository.class).should()
  ...
  /**
   * DevonNamingConventionCheck N5 verifying that non-abstract classes inherited
   * from AbstractUc are following the
   * devonfw naming convention by beginning with 'Uc' and ending with 'Impl'. They
   * must also implement an interface with
   * the same name except for the suffix 'Impl'.
   */
  static ArchRule namingConventionUcCheck() {
    return classes().that().areAssignableTo(AbstractUc.class).and()
        .doNotHaveModifier(JavaModifier.ABSTRACT).should().haveSimpleNameStartingWith("Uc")
